...
for name, model in models.items():
    ...
    # simpan acc per model key
    if "Standar" in name:
        acc_per_model["model1_acc"] = float(acc)
    else:
        acc_per_model["model2_acc"] = float(acc)
    ...
    plt.ylabel("Actual")
    plt.tight_layout()
    suffix = "1" if "Standar" in name else "2"
    plt.savefig(f"confusion_matrix_{suffix}.png", dpi=150, bbox_inches="tight")
    plt.show()

with open("test_metrics.pkl", "wb") as f:
    pickle.dump(acc_per_model, f)

print(f"   model1_acc = {acc_per_model.get('model1_acc', 0):.4f}")
print(f"   model2_acc = {acc_per_model.get('model2_acc', 0):.4f}")
...
for pkl_file, model_name in [
    ("history_model1.pkl", "Model 1 — ANN Standar"),
    ("history_model2.pkl", "Model 2 — Deep ANN")
]:
...
for pkl_file, mname in [
    ("cv_scores_model1.pkl", "Model 1"),
    ("cv_scores_model2.pkl", "Model 2")
]:
    try:
        with open(pkl_file, "rb") as f:
...
print("\nEvaluasi selesai. Semua file gambar tersimpan.")